...
class SellerLotViewAnimator: NSObject {
...
extension SellerLotViewAnimator: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        if let fromVC = transitionContext.viewController(forKey: .from) as? CatalogView,
            let toVC = transitionContext.viewController(forKey: .to) as? SellerLotView {
            self.animateTransitionFromView(fromVC: fromVC, toVC: toVC, using: transitionContext)
        } else if let fromVC = transitionContext.viewController(forKey: .from) as? SellerLotView,
    ...
    private func animateTransitionFromView(fromVC: CatalogView,
        ...
        UIView.animate(withDuration: self.duration*0.3, delay: self.duration*0.1, options: .curveEaseInOut, animations: {
            ...
            transitionImageViewBackground.layer.cornerRadius = 8
        ...
        UIView.animate(withDuration: self.duration*0.2, delay: self.duration*0.3, options: .curveEaseInOut, animations: {
    ...
    private func animateTransitionFromView(fromVC: SellerLotView,
                                           toVC: CatalogView,
                                           using transitionContext: UIViewControllerContextTransitioning) {
        guard let fromView = fromVC.view,
            let toView = toVC.view,
        ...
        UIView.animate(withDuration: self.duration*0.1, delay: 0, options: .curveEaseInOut, animations: {
            ...
            transitionImageViewBackground.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
            transitionImageView.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
        }

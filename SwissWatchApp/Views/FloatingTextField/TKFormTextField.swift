...
open class TKFormTextField: UITextField {
    ...
    // MARK: Animation
    open var titleFadeInDuration: TimeInterval = 0.1
    open var titleFadeOutDuration: TimeInterval = 0.15
    ...
    fileprivate func updateLineColor() {
        ...
        if self.hasError {
            color = self.errorLineColor
        } else {
            color = self.editingOrSelected ? self.selectedLineColor : self.lineColor
        }
        self.lineView.backgroundColor = color
    }
    
    fileprivate func updateTitleColor() {
        let color: UIColor = self.editingOrSelected ? self.selectedTitleColor : self.titleColor
//        if self.hasError {
//            color = self.errorColor
//        } else {
//            color = self.editingOrSelected ? self.selectedTitleColor : self.titleColor
//        }
...
//        self.titleColor = color
    ...
    open func titleLabelRectForBounds(_ bounds: CGRect, editing:Bool) -> CGRect {
        ...
        if editing {
            return CGRect(x: 0, y: 0 + verticalInset, width: bounds.size.width, height: titleHeight)
        }
        return CGRect(x: 0, y: titleHeight + verticalInset, width: bounds.size.width, height: titleHeight)
    }
